...
parser.add_argument("--model_name", type=str, default="ViT-B/32",choices=["ViT-B/32", "RN50", "RN50x64", "ViT-L/14@336px"])
...
def load_dataset(dataset_type):
    ds: DenseCaptionedDataset = get_complete_dataset_with_settings(
        split=dataset_type, 
    ...
    if os.path.exists(text_db_dir):
        ...
        with torch.no_grad():
            for i, sample in enumerate(ds):  
                ...
                partial_caps = [s+"." for s in global_set["caption"].split(". ")]
                if text_type == "global" or text_type == "hybrid" or len(partial_caps) == 0:
                    g_caption = global_set["caption"]
                ...
                if (text_type == "partial" or text_type == "hybrid") and len(partial_caps) > 0:                    
                    for p_caption in partial_caps:
